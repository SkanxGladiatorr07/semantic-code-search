...
class AIService {
  ...
  initialize() {
    ...
    console.log('Initializing OpenAI with API key:', this.apiKey.substring(0, 15) + '...');
  ...
  async generateText(prompt) {
    try {
      ...
      throw new Error(`Failed to generate text from OpenAI: ${error.message || error}`);
    }
  }
  ...
  async generateTextStream(prompt) {
    try {
      ...
      throw new Error('Failed to generate text stream from OpenAI');
    }
  }
